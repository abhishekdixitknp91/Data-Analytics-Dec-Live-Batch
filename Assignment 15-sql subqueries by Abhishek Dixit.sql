...
FROM sales WHERE emp_id IN(SELECT emp_id FROM employees WHERE salary >(SELECT AVG(salary)FROM employees));
...
(SELECT department_id FROM employees WHERE emp_id IN(SELECT emp_id FROM sales GROUP BY emp_id HAVING SUM(sale_amount) > 10000));

-- 4.Find the employee who has made the second-highest sale
SELECT *
FROM employees WHERE emp_id =
(SELECT emp_id FROM sales WHERE sale_amount <(SELECT MAX(sale_amount) FROM sales)ORDER BY sale_amount DESC LIMIT 1);
