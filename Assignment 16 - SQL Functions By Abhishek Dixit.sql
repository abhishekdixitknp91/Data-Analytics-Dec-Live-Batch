...
SELECT UPPER(name) AS student_name_upper, MONTHNAME(join_date) AS join_month 
FROM Student_Performance;

-- Question 4: Show name and next student's attendance
SELECT name, attendance, LEAD(attendance) OVER (ORDER BY score desc) AS next_student_attendance
FROM Student_Performance;
...
SELECT name, course,attendance,ROW_NUMBER() OVER (PARTITION BY course ORDER BY attendance DESC) AS attendance_rank
FROM Student_Performance;
...
SELECT name,course,score,FIRST_VALUE(score) OVER (PARTITION BY course ORDER BY score DESC
